...
class Scheduler<T> {
    ...
    Scheduler() {
        this.size = 10;
        this.timestamp = (Timestamp<T>[]) new Timestamp[size];
        this.elementsInside = 0;
    }
    ...
    void add(Timestamp<T> t) {
        if (elementsInside == size) {
            int oldSizee = size;
            size *= 2;
            Timestamp<T>[] newArr = (Timestamp<T>[]) new Timestamp[size];
            System.arraycopy(timestamp, 0, newArr, 0, oldSizee);
            timestamp = newArr;
        }
        timestamp[elementsInside++] = t;
    }
